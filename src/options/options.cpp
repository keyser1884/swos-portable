...
std::vector<LogItem> parseCommandLine(int argc, char **argv)
{
    ...
    const char kCompareMode[] = "--compare-mode";
    const char kBreakOnMismatch[] = "--break-on-mismatch";
    const char kQuickMatch[] = "--quick-match";
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strcmp(argv[i], kCompareMode)) {
            g_comparePortedFunctions = true;
            initComparisonLog();
            log("Compare mode enabled via command line", kInfo);
        } else if (!strcmp(argv[i], kBreakOnMismatch)) {
            g_breakOnMismatch = true;
            log("Break on mismatch enabled via command line", kInfo);
        } else if (!strcmp(argv[i], kQuickMatch)) {
            g_quickMatchMode = true;
            log("Quick match mode enabled via command line", kInfo);
        } else {
