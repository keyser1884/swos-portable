...
SWOS::RngState SWOS::saveRngState()
{
    RngState state;
    state.seed = swos.seed;
    state.xorKey = m_xorKey;
    state.xorIndex = m_xorIndex;
    return state;
}

void SWOS::restoreRngState(const RngState& state)
{
    swos.seed = state.seed;
    m_xorKey = state.xorKey;
    m_xorIndex = state.xorIndex;
}
