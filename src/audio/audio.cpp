...
bool usingCustomAudioDir()
{
    static int s_hasCustomAudioDir = -1;
    if (s_hasCustomAudioDir < 0) {
        const auto audioPath = pathInRootDir(kAudioDir);
        s_hasCustomAudioDir = dirExists(audioPath.c_str()) ? 1 : 0;
    }

    return s_hasCustomAudioDir == 1;
}
